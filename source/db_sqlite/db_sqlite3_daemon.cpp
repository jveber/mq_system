...
SQLite_DB_Service::SQLite_DB_Service(): _tokener(json_tokener_new()), Daemon("mq_db_daemon", "/var/run/mq_db_daemon.pid") {}
...
void SQLite_DB_Service::load_daemon_configuration() {
    ...
    try {
        ...
        for (auto db_element = db_elements.begin(); db_element != db_elements.end(); ++db_element) {
            std::string sensor_name = db_element->lookup("name");
            ...
            for (const auto& db_value : db_values) {
                // value name
                std::string value_name = db_value.lookup("name");
                // value averaging
                bool value_averaging = false;
                if (db_value.exists("averaging"))
                    value_averaging = db_value.lookup("averaging");
                // value interval
                uint64_t value_interval = 0;
                if (db_value.exists("interval")) {
                    ...
                    switch (value_type.getType()) {
                        case Setting::Type::TypeInt:
                        {
                            ...
                            value_interval = static_cast<uint64_t>(int_value) * 1000000000L;
                            break;
                        }
                        case Setting::Type::TypeInt64:
                        {
                            uint64_t long_value = static_cast<uint64_t>(value_type);
                            value_interval = static_cast<uint64_t>(long_value) * 1000000000L;
                            break;
                        }
                        case Setting::Type::TypeFloat:
                        {
                            float float_value = static_cast<float>(value_type);
                            value_interval = static_cast<uint64_t>(float_value) * 1000000000L;
                            break;
                        }
                        default:
                            _logger->warn("Value type not handled - interval");
                            break;
                    }
                }
                double precision = 0.0;
                if (db_value.exists("precision")){
                    precision = db_value.lookup("precision");
                }
...
void SQLite_DB_Service::CallBack(const std::string& topic, const std::string& message) {
    ...
    auto search_sensor_name_id_result = get_name_id(_known_sensors, message_sensor_name, sensor_insert_stmt, sensor_req_stmt);
    ...
    json_object_object_foreach(message_json_root_object, message_cstr_value_name, message_json_value_data) {
        const auto message_value_name = std::string(message_cstr_value_name);
        ...
        auto&& ns_since_first_value_update = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(now - current_value_data.last_update).count());
        ...
        if (json_object_get_type(message_json_value_data) == json_type_array) {
            unit_name = json_object_get_string(json_object_array_get_idx (message_json_value_data, 1));   //uints are second element in array
            message_json_value_data = json_object_array_get_idx (message_json_value_data, 0);             //while first is the value
        }
            ...
            switch (json_object_get_type(message_json_value_data)) {
                ...
                case json_type_double:
                {
                    ...
                    if (current_value_data.averaging && current_value_data.ValEvents.size()) {
                        ...
                        auto total_time_elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(now - last_event_time_mark).count();                        
                        ...
                        for (auto ValEventIterator = current_value_data.ValEvents.cbegin() + 1; ValEventIterator != current_value_data.ValEvents.cend(); ++ValEventIterator) {
                            auto event_time_elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(ValEventIterator->time_mark - last_event_time_mark).count();                            
                            average_value += last_event_value * (static_cast<double>(event_time_elapsed) / static_cast<double>(total_time_elapsed));
                            if (current_value_data.ValEvents.size() < 10)
...
int main() {
    try {
        SQLite_DB_Service *d = new SQLite_DB_Service();
        d->main();
        delete d;
